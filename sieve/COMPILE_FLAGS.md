...
| `NARROW_ENTRY` *(make)* | 1 | Bucket entry format. Narrow entries are 4-byte prime-only, with one divide per hit; fastest on many-core ARM. Set to 0 for wide 8-byte packed entries (divide-free per hit), which may win on x86 where the divide is costlier. Becomes `-DSIEVE_NARROW_ENTRY`. |
