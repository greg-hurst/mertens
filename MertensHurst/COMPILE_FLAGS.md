...
| `LOOP2_SIEVE_P` | 1 | Compile-time Loop-2 sieve and default-parameter selector. `1` retains full $M$ with `(nuRatio,u-anchor)=(0.90,0.55@10^16)`, while `2` uses the packed odd-prefix identity with `(0.95,0.70@10^18)`; both u-factors fall by 0.025 per decade with a 0.30 floor. Use the contract-closed `q210-coupled-loop2-p1` and `-p2` targets; a failed runtime Q210 guard in the P2 binary retains the full-$M$ backend with the configured P2 parameters. |
